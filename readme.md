...
- с простым и понятным `service.bat` (авто‑проверка обновлений и обновление списков/бинарников из форка).
...
2. Скачайте архив (zip/rar) со [страницы последнего релиза](https://github.com/Vanilla-SilQ-HD/Zapretik/releases/latest)
...
- [**`service.bat`**](./service.bat) - установка в автозапуск и сервисное меню:
  ...
  - **`Update bin files`** — загрузка/обновление `winws.exe` и WinDivert (`WinDivert.dll`, `WinDivert64.sys`, `cygwin1.dll`) из [`zapret-win-bundle/zapret-winws`](https://github.com/bol-van/zapret-win-bundle/tree/master/zapret-winws);
  - **`Run Diagnostics`** — диагностика частых проблем с запусками/службами/правами;
  - **`Run Tests`** — отдельное окно с тестами стратегий (`utils/test zapret.ps1`: http/https‑тесты и DPI‑чекеры).
...
- Скачайте последнюю версию [со страницы релизов zapretik](https://github.com/Vanilla-SilQ-HD/Zapretik/releases) (`zapretik-...`)
